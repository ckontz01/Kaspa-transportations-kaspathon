    ...
    if (!verify_csrf_token($token)) {
        ...
        if (!$errors) {
            ...
            if ($stmt === false) {
                ...
                redirect('passenger/messages.php?user_id=' . urlencode((string)$targetUserId));
            }
        }
    }
}

// Get all conversations for the passenger (sorted by unread first)
$conversations = [];
$stmt = db_call_procedure('dbo.spGetPassengerConversations', [$currentUserId]);
if ($stmt !== false) {
...
// Get all drivers this passenger can contact (from trip history)
$contactableDrivers = [];
$stmt = db_call_procedure('dbo.spGetPassengerContactableDrivers', [$currentUserId]);
if ($stmt !== false) {
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        $contactableDrivers[] = $row;
    }
    sqlsrv_free_stmt($stmt);
}
...
function format_msg_time_passenger($v): string {
...
function format_last_msg_preview_passenger($content, $maxLen = 35): string {
...
<div class="messages-container">
    <div class="messages-header">
        <h1>💬 Messages</h1>
        <p>Chat with support or your drivers</p>
    </div>
    ...
    <div class="messages-layout">
        ...
        <div class="conversations-panel">
            <div class="conversations-header">
                ...
                // Add drivers from trip history not in conversations
                foreach ($contactableDrivers as $driver) {
                    if (!in_array($driver['UserID'], $existingUserIds)) {
                        // Ensure UserRole is set for dropdown label
                        if (!isset($driver['UserRole'])) {
                            $driver['UserRole'] = 'Driver';
                        }
                        $newContactOptions[] = $driver;
                    }
                }
                ...
                document.getElementById('newConversationSelect').addEventListener('change', function() {
                    if (this.value) {
                        window.location.href = '<?php echo e(url('passenger/messages.php?user_id=')); ?>' + this.value;
                    }
                });
                </script>
                <?php endif; ?>
            ...
            <div class="conversations-list">
                <?php if (empty($conversations) && empty($newContactOptions)): ?>
                    <div class="no-conversations">
                        ...
                        <p style="font-size: 0.78rem; margin-top: 0.5rem;">Complete a trip to message your driver.</p>
                    </div>
                ...
                <?php else: ?>
                    <?php foreach ($conversations as $conv): ?>
                        ...
                        $roleClass = $role === 'driver' ? 'driver' : 'support';
                        ?>
                        <a href="<?php echo e(url('passenger/messages.php?user_id=' . $conv['UserID'])); ?>" 
                           class="conversation-item <?php echo $isActive ? 'active' : ''; ?> <?php echo $hasUnread ? 'unread' : ''; ?>">
                            ...
                            <div class="conversation-info">
                                ...
                                <div class="conversation-preview">
                                    <?php echo e(format_last_msg_preview_passenger($conv['LastMessage'] ?? '')); ?>
                                </div>
                            </div>
                            <div class="conversation-meta">
                                <div class="conversation-time"><?php echo e(format_msg_time_passenger($conv['LastMessageAt'] ?? null)); ?></div>
                                <?php if ($hasUnread): ?>
                    ...
                    <?php else: ?>
                        <?php foreach ($messages as $msg): ?>
                            ...
                            <div class="message-bubble <?php echo $isMine ? 'outgoing' : 'incoming'; ?>">
                                ...
                                <div class="message-time"><?php echo e(format_msg_time_passenger($msg['SentAt'] ?? null)); ?></div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
