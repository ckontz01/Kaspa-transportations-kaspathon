...
BEGIN
    ...
    -- Calculate fare breakdown (20% service fee)
    DECLARE @ServiceFeeRate DECIMAL(5,4) = 0.2000;
    DECLARE @ServiceFeeAmount DECIMAL(10,2) = @Amount * @ServiceFeeRate;
    DECLARE @DriverEarnings DECIMAL(10,2) = @Amount - @ServiceFeeAmount;
...
BEGIN
    ...
    SELECT
        ...
        ISNULL(SUM(r.TotalCost) * 0.20, 0) AS TotalServiceFees
...
BEGIN
    ...
    SELECT
        ...
        -- Calculate estimated driver earnings (80% of total)
            ...
            WHEN rs.SegmentID IS NOT NULL THEN
                ...
                CASE 
                    WHEN segPay.Status = 'completed' THEN
                        ...
                        COALESCE(segPay.DriverEarnings, ROUND(segPay.Amount * 0.80, 2))
                    ELSE
            ...
            ELSE
                ...
                CASE 
                    WHEN pay.Status = 'completed' THEN
                        COALESCE(pay.DriverEarnings, ROUND(pay.Amount * 0.80, 2))
                    ELSE
                        ROUND(COALESCE(rr.EstimatedFare, 0) * 0.80, 2)
                END
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT 
            ...
            @ServiceFeeRate = ISNULL(pc.ServiceFeeRate, 0.2000),
        ...
        IF @ServiceFeeRate IS NULL SET @ServiceFeeRate = 0.2000;
...
BEGIN
    ...
    SELECT 
        ...
        -- Calculate estimated driver payment (80% of total fare) using actual distance
        ROUND(
            CASE 
                WHEN (
                ...
                THEN COALESCE(pc.MinimumFare, CASE rr.ServiceTypeID 
                    WHEN 1 THEN 5.00 WHEN 2 THEN 15.00 WHEN 3 THEN 10.00 
                    WHEN 4 THEN 20.00 WHEN 5 THEN 8.00 ELSE 5.00 END) * 0.80
                ELSE (
                    COALESCE(pc.BaseFare, CASE rr.ServiceTypeID 
                        WHEN 1 THEN 3.00 WHEN 2 THEN 8.00 WHEN 3 THEN 5.00 
                        WHEN 4 THEN 10.00 WHEN 5 THEN 4.00 ELSE 3.00 END) +
...
BEGIN
    ...
    SELECT 
        ...
        ROUND(ISNULL(rs.EstimatedFare, 0) * 0.80, 2) AS EstimatedDriverPayment,
...
BEGIN
    ...
    DECLARE @DriverEarnings DECIMAL(10,2) = ROUND(@FinalFare * 0.80, 2);
    DECLARE @PlatformFee DECIMAL(10,2) = ROUND(@FinalFare * 0.20, 2);
...
BEGIN
    ...
    DECLARE @ServiceFeeRate DECIMAL(5,4) = 0.20; -- 20% platform fee
    ...
    SELECT 
        ...
        SUM(p.Amount * @ServiceFeeRate) as TotalServiceFees,
        SUM(p.Amount * (1 - @ServiceFeeRate)) as TotalDriverPayouts,
...
BEGIN
    ...
    DECLARE @ServiceFeeRate DECIMAL(5,4) = 0.20;
    
    SELECT 
        CAST(p.CreatedAt AS DATE) as PaymentDay,
        COUNT(*) as PaymentCount,
        SUM(p.Amount) as DailyRevenue,
        SUM(p.Amount * @ServiceFeeRate) as DailyServiceFees,
        SUM(p.Amount * (1 - @ServiceFeeRate)) as DailyDriverPayouts
    FROM dbo.Payment p
    WHERE p.Status = 'completed'
      AND p.CreatedAt >= @StartDate
      AND p.CreatedAt <= DATEADD(DAY, 1, @EndDate)
...
BEGIN
    ...
    SELECT 
        ...
        SUM(p.Amount * @ServiceFeeRate) as ServiceFees
    FROM dbo.Payment p
    INNER JOIN dbo.PaymentMethodType pmt ON p.PaymentMethodTypeID = pmt.PaymentMethodTypeID
    WHERE p.Status = 'completed'
      AND p.CreatedAt >= @StartDate
...
BEGIN
    ...
    SELECT TOP (@TopN)
        ...
        SUM(p.Amount * @ServiceFeeRate) as ServiceFeesPaid,
        SUM(p.Amount * (1 - @ServiceFeeRate)) as NetEarnings
    FROM dbo.Payment p
    INNER JOIN dbo.Trip t ON p.TripID = t.TripID
    INNER JOIN dbo.Driver d ON t.DriverID = d.DriverID
    INNER JOIN dbo.[User] u ON d.UserID = u.UserID
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(SUM(p.ServiceFeeAmount), SUM(p.Amount * 0.20), 0) as TotalServiceFee,
        COALESCE(SUM(p.DriverEarnings), SUM(p.Amount * 0.80), 0) as TotalNet,
    ...
    SELECT 
        ...
        COALESCE(p.ServiceFeeAmount, p.Amount * 0.20) as ServiceFeeAmount,
        COALESCE(p.ServiceFeeRate, 0.20) as ServiceFeeRate,
        COALESCE(p.DriverEarnings, p.Amount * 0.80) as DriverEarnings,
        p.Status,
...
BEGIN
    ...
    SELECT 
        ...
        @PricePerMinute = ISNULL(pc.PricePerMinute, 0.20),
        @MinimumFare = ISNULL(pc.MinimumFare, 5.00),
        @ServiceFeeRate = ISNULL(pc.ServiceFeeRate, 0.20),
    ...
    IF @PricePerMinute IS NULL SET @PricePerMinute = 0.20;
    IF @MinimumFare IS NULL SET @MinimumFare = 5.00;
    IF @ServiceFeeRate IS NULL SET @ServiceFeeRate = 0.20;
...
BEGIN
    ...
    SET @TotalFare = dbo.fnCalculateTotalFare(@ServiceTypeID, @DistanceKm, @DurationMin);
    
    SELECT @ServiceFeeRate = pc.ServiceFeeRate
    FROM dbo.PricingConfig pc
    WHERE pc.ServiceTypeID = @ServiceTypeID AND pc.IsActive = 1;
    
    IF @ServiceFeeRate IS NULL
        SET @ServiceFeeRate = 0.2000;
    
    RETURN ROUND(@TotalFare * (1 - @ServiceFeeRate), 2);
END
GO
...
BEGIN
    ...
    SELECT 
        ...
        -- Driver payment (80% of segment fare) with fallback calculation
        ROUND(
            COALESCE(rs.EstimatedFare,
                ROUND(
                    CASE rr.ServiceTypeID
...
BEGIN
    ...
    SELECT 
        ...
        ISNULL(SUM(r.TotalDurationMin), 0) AS TotalMinutesRented,
        ISNULL(SUM(r.DistanceKm), 0) AS TotalDistanceKm,
        ISNULL(SUM(r.TotalCost) * 0.20, 0) AS ServiceFees,  -- 20% platform fee
        COUNT(DISTINCT r.CustomerID) AS UniqueCustomers,
...
BEGIN
    ...
    SELECT 
        ...
        ISNULL(SUM(r.TotalCost) * 0.20, 0) AS DailyServiceFees,
...
BEGIN
    ...
    SELECT TOP (@TopN)
        ...
        ISNULL(SUM(r.TotalCost) * 0.20, 0) AS ServiceFees,
        ISNULL(SUM(r.TotalDurationMin), 0) AS TotalMinutesUsed,
        ISNULL(SUM(r.DistanceKm), 0) AS TotalDistanceKm
    FROM dbo.CarshareRental r
    JOIN dbo.CarshareVehicle v ON r.VehicleID = v.VehicleID
